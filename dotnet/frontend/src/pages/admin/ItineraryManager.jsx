...
const ItineraryManager = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingItinerary) {
                await axios.put(`${BACKEND_URL}/api/itineraries/${editingItinerary.itinerary_id}`, formData);
            } else {
    ...
    return (
        <div className="admin-page">
            <div className="page-header">
                ...
                <button className="btn-primary" onClick={() => { setShowModal(true); setEditingItinerary(null); setFormData({ itinerary_id: '', day_number: '', itinerary_details: '', catmaster: { categoryId: '' } }) }}>
                    Add Day
                </button>
            </div>
                ...
                <table className="admin-table">
                    ...
                    <tbody>
                        {itineraries.map(itin => (
                            <tr key={itin.itinerary_id}>
                                <td>{itin.itinerary_id}</td>
                                <td>{itin.catmaster?.name || 'Unknown'}</td>
                                <td>Day {itin.day_number}</td>
                                <td>{itin.itinerary_details?.substring(0, 50)}...</td>
                                <td>
                                    <button className="btn-edit" onClick={() => { setEditingItinerary(itin); setFormData(itin); setShowModal(true); }}>Edit</button>
                                    <button className="btn-delete" onClick={() => deleteItinerary(itin.itinerary_id)}>Delete</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
                        ...
                        <form onSubmit={handleSubmit}>
                            <div className="form-group">
                                ...
                                >
                                    ...
                                    {categories.map(c => (
                                        <option key={c.categoryId || c.category_id} value={c.categoryId || c.category_id}>{c.name}</option>
                                    ))}
                                </select>
                            </div>
                            <div className="form-group">
                                <label>Day Number</label>
                                <input type="number" value={formData.day_number} onChange={e => setFormData({ ...formData, day_number: e.target.value })} required />
                            </div>
                            <div className="form-group">
                                <label>Itinerary Details</label>
                                <textarea
                                    value={formData.itinerary_details}
                                    onChange={e => setFormData({ ...formData, itinerary_details: e.target.value })}
                                    required
