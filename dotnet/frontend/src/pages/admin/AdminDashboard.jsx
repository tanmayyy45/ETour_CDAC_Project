        ...
        <div className="admin-page">
            ...
            <div className="dashboard-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '1.5rem', marginBottom: '2rem' }}>
                <div className="stat-card" style={{ background: 'white', padding: '1.5rem', borderRadius: '12px', boxShadow: '0 4px 15px rgba(0,0,0,0.05)' }}>
                    <h4 style={{ margin: '0 0 1rem 0', color: '#7f8c8d' }}>Total Revenue</h4>
                    <p style={{ fontSize: '2rem', margin: 0, fontWeight: 'bold', color: '#27ae60' }}>
                        {loading ? '--' : `₹${stats.totalRevenue?.toLocaleString()}`}
                    </p>
                </div>
                <div className="stat-card" style={{ background: 'white', padding: '1.5rem', borderRadius: '12px', boxShadow: '0 4px 15px rgba(0,0,0,0.05)' }}>
                    <h4 style={{ margin: '0 0 1rem 0', color: '#7f8c8d' }}>Total Bookings</h4>
                    <p style={{ fontSize: '2rem', margin: 0, fontWeight: 'bold', color: '#2c3e50' }}>{loading ? '--' : stats.totalBookings}</p>
                    <div style={{ display: 'flex', gap: '1rem', marginTop: '0.5rem', fontSize: '0.9rem' }}>
                        <span style={{ color: '#f39c12' }}>⏳ {stats.pendingBookings} Pending</span>
                        <span style={{ color: '#27ae60' }}>✅ {stats.confirmedBookings} Confirmed</span>
                    </div>
                </div>
                <div className="stat-card" style={{ background: 'white', padding: '1.5rem', borderRadius: '12px', boxShadow: '0 4px 15px rgba(0,0,0,0.05)' }}>
                    <h4 style={{ margin: '0 0 1rem 0', color: '#7f8c8d' }}>Upcoming Departures</h4>
                    <p style={{ fontSize: '2rem', margin: 0, fontWeight: 'bold', color: '#3498db' }}>{loading ? '--' : stats.upcomingDepartures}</p>
                </div>
                <div className="stat-card" style={{ background: 'white', padding: '1.5rem', borderRadius: '12px', boxShadow: '0 4px 15px rgba(0,0,0,0.05)' }}>
