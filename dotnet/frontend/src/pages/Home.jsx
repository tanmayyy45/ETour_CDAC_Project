...
const Home = () => {
    ...
    const handleSearch = (searchParams) => {
        ...
        // UNIFIED SEARCH: Supports combining filters (Dest + Date + Budget)
        // And returns CategoryDTOs which have valid ImagePaths

        // Sanitize params: Remove empty strings to prevent backend issues
        const cleanParams = Object.fromEntries(
            Object.entries(searchParams).filter(([_, v]) => v !== '' && v !== null)
        );

        searchCategories(cleanParams)
            .then(res => {
                setSearchResults(res.data);
                setResultType('category'); // Always use CategoryCard for consistency
            })
            .catch(err => {
